/**
 * Route Handler: GET /api/MOD05/reporteRhResultados
 *
 * Propósito: Obtener resultados filtrados del reporte RRHH.
 */

import { NextRequest, NextResponse } from 'next/server';
import { graphqlRequest } from '@/api/graphql/client';
import {
    GET_REPORTE_RH_RESULTADOS_QUERY,
    EstatusEmpleadoRh,
    EstatusEmpleadoRhEnum,
    CampoEmpleadoRh,
} from '@/api/graphql/MOD05';
import type { GetReporteRhResultadosResponse, ReporteRhInput } from '@/api/graphql/MOD05';
...
export async function GET(request: NextRequest) {
    try {
        ...
        const typedResponse = response as unknown as GetReporteRhResultadosResponse;

        return NextResponse.json(
            {
                data: typedResponse.reporteRhResultados,
            },
            { status: 200 },
        );
    } catch (error) {
